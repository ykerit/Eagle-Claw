...
{
    GLCALL(glCreateVertexArrays(1, &renderID_));
}
    ...
    for (const auto &element : layout)
    {
        ...
        case BufferDataType::Bool: {
            ...
            glVertexAttribPointer(vertexBufferIdx_, element.GetComponentCount(), BufferDateType2GLType(element.type),
                element.normalized ? GL_TRUE : GL_FALSE, layout.GetStride(), (void *)element.offset);
            ++vertexBufferIdx_;
            break;
        }
        case BufferDataType::Mat3:
        case BufferDataType::Mat4: {
            ...
            for (uint8_t i = 0; i < count; ++i)
            {
                ...
                glVertexAttribPointer(vertexBufferIdx_, count, BufferDateType2GLType(element.type), element.normalized ? GL_TRUE : GL_FALSE,
                    layout.GetStride(), (void *)(element.offset + sizeof(float) * count + i));
