...
GLShader::GLShader(const std::string &name, const std::string &vertSrc, const std::string &fragmentSrc)
    : name_(name)
{
    std::unordered_map<GLenum, std::string> sources;
...
std::string GLShader::ReadFile(const std::string &file)
{
    ...
    if (in)
    {
        ...
        {
            GD_CORE_ERROR("Could not read from file: {}", file);
        }
    }
    else
    {
        GD_CORE_ERROR("Could not open file: {}", file);
    }
    return result;
}
